...
################################################################################
# CDP Master Startup Script
# 
# This script orchestrates the startup of CDP resources in the proper order:
# 1. Environments (start)
# 2. DataHubs (start) - 1 hour after Environment
# 3. ML Workspaces (resume) - 1 hour after DataHubs
#
# All configuration is done via variables at the top of this script.
# NOTE: Startup order is REVERSE of shutdown order!
################################################################################
...
LOG_DIR="./logs"
LOG_FILE="${LOG_DIR}/master-startup-$(date '+%Y%m%d-%H%M%S').log"

# Script paths (relative to this script's location)
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ENVIRONMENT_SCRIPT="${SCRIPT_DIR}/start-environments.sh"
DATAHUB_SCRIPT="${SCRIPT_DIR}/start-datahubs.sh"
ML_WORKSPACE_SCRIPT="${SCRIPT_DIR}/start-ml-workspaces.sh"
...
log_section "CDP Master Startup Script Started"
...
if [ ${#PHASES_TO_RUN[@]} -eq 0 ]; then
    log "ERROR: No resources specified for startup!"
...
if [ -n "$ENVIRONMENT_NAME" ]; then
    log_section "PHASE 1: Starting Environment"
    
    if execute_script "$ENVIRONMENT_SCRIPT" "$ENVIRONMENT_NAME"; then
        log "Environment startup phase completed successfully"
    else
        log "ERROR: Environment startup encountered errors"
        exit 1
    fi
    
    PREVIOUS_PHASE_RAN=true
else
    log_section "PHASE 1: Skipping Environment (not configured)"
fi

################################################################################
# PHASE 2: Start DataHubs
################################################################################

if [ -n "$DATAHUB_NAMES" ]; then
    # Wait if a previous phase ran
    if [ "$PREVIOUS_PHASE_RAN" = true ]; then
        log_section "WAIT PERIOD: Before DataHub Startup"
        wait_with_countdown "$DELAY_BETWEEN_PHASES" "DataHub startup"
    fi
    
    log_section "PHASE 2: Starting DataHubs"
    
    if execute_script "$DATAHUB_SCRIPT" "$DATAHUB_NAMES"; then
        log "DataHub startup phase completed successfully"
    else
        log "WARNING: DataHub startup encountered errors, but continuing..."
    fi
...
################################################################################
# PHASE 3: Resume ML Workspaces
################################################################################

if [ -n "$ML_WORKSPACE_CRNS" ]; then
    # Wait if a previous phase ran
    if [ "$PREVIOUS_PHASE_RAN" = true ]; then
        log_section "WAIT PERIOD: Before ML Workspace Resume"
        wait_with_countdown "$DELAY_BETWEEN_PHASES" "ML Workspace resume"
    fi
    
    log_section "PHASE 3: Resuming ML Workspaces"
    
    if execute_script "$ML_WORKSPACE_SCRIPT" "$ML_WORKSPACE_CRNS"; then
        log "ML Workspaces resume phase completed successfully"
    else
        log "WARNING: ML Workspaces resume encountered errors, but continuing..."
    fi
else
    log_section "PHASE 3: Skipping ML Workspaces (not configured)"
fi
...
log_section "CDP Master Startup Script Completed Successfully"
...
log "  Log file: $LOG_FILE"
log ""
log "All configured CDP resources have been started successfully."
log "Resources are now available for use."
