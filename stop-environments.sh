...
if [[ $interactive -eq 1 ]]; then
    read -p "Do you want to proceed with shutting down specific environments? (Y/N): " proceed

    if [[ $proceed =~ ^[Yy]$ ]]; then
        read -p "Enter the indexes of the environments to shut down (e.g., 1,3,5): " indexes
    ...
    # Automatically select all provided environments for shutting down, skipping any with PERMISSION_DENIED status
    for i in "${!validEnvironmentNames[@]}"; do
...
# Shutting down selected environments
for index in "${selectedEnvironmentsIndexes[@]}"; do
    environmentName="${validEnvironmentNames[$index]}"
    echo "Initiating shutdown for $environmentName..."
    cdp environments stop-environment --environment-name "$environmentName"
done

if [ ${#selectedEnvironmentsIndexes[@]} -gt 0 ]; then
    echo "Monitoring the shutdown process for the selected environments..."
    while :; do
        allStopped=true
        ...
        if [[ "$allStopped" = true ]]; then
            echo "All selected environments have been successfully stopped."
            break
        fi
        sleep 15
    done
